...
Блоки `infrastructure` и `bootstrap` — **плоский `map[string]any`**, без вложенности. Каждый ключ маппится 1:1 на `{{ .ключ }}` в соответствующем шаблоне. Набор ключей произвольный — CRD не фиксирует конкретные имена переменных.
...
|----------|----------|
| Тип | `map[string]any` |
| Вложенность | Запрещена. Значения могут быть string/bool/array/object |
